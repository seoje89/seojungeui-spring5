            ...
            <fieldset>
                ...
                <ul class="app_list">
                    <li class="clear">
                        ...
                        <div class="app_content"><input value ="${memberVO.user_id}" readonly type="text" name="user_id" class="w100p" id="user_id_lbl" placeholder="ID를 입력해주세요" required />
                    </li>
                    <li class="clear">
                        <label for="password_lbl" class="tit_lbl pilsoo_item">비밀번호</label>
                        <div class="app_content"><input type="password" name="user_pw" class="w100p" id="password_lbl" placeholder="비밀번호를 입력해주세요" />
                    </li>
                    <li class="clear">
                        <label for="user_name_lbl" class="tit_lbl pilsoo_item">사용자이름</label>
                        <div class="app_content"><input value="${memberVO.user_name}" type="text" name="user_name" class="w100p" id="user_name_lbl" placeholder="사용자이름을 입력해주세요" required />
                    </li>
                    <li class="clear">
                        <label for="email_lbl" class="tit_lbl pilsoo_item">이메일</label>
                        <div class="app_content"><input value="${memberVO.email}" type="email" name="email" class="w100p" id="email_lbl" placeholder="이메일을 입력해주세요" required />
                    </li>
                    <li class="clear">
                        <label for="point_lbl" class="tit_lbl pilsoo_item">포인트</label>
                        <div class="app_content"><input value="${memberVO.point}" readonly type="digits" name="point" class="w100p" id="point_lbl" placeholder="포인트를 입력해주세요" required />
                    </li>
                    <li class="clear">
...
$(document).ready(function(){
	$("#btn_leave").click(function(){
		if(confirm('정말로 탈퇴하시겠습니까?')){
			...
			//위 값을 false 또는 0 으로 바꾼 뒤 submit 예정
			form_leave.attr("action","/member/mypage_leave"); // 크롬 오동작으로 추가한 코드
			form_leave.submit(); // 삭제는 아니고, enabled 필드값을 수정
			//탈퇴를 했다면, 로그아웃처리도 동시에 되어야 함
			//location.replace("/logout"); //크롬에서 오동작하기 때문에 javascript 처리 안함
		}
	});
});
</script>