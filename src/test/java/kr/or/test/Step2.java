...
class MemberVO {
	...
	//하나씩 출력하기 힘들때 자동으로 멤버변수값을 출력하는 명령 toString 생성
	@Override
	public String toString() {
		return "MemberVO 출력 [name=" + name + ", age=" + age + ", phoneNum=" + phoneNum + "]";
	}
	//위 private 멤버변수를 입출력하는 구현 메서드를 만듭니다
	...
	// void한(반환값이 없는) 입력하는 셋 메서드(아래)
	public void setPhoneNum(String phoneNum) {
		this.phoneNum = phoneNum;
		//프라이빗 변수를 직접접근하지 못하니 퍼블릭 메서드로 입력하게 만들어놓은 구조이다.
	}
...
public class Step2 {
	...
	public static void main(String[] args) {
		...
		//MemberVO 클래스를 배열 객체로 생성했음(아래)
		MemberVO[] members = new MemberVO[3];
		members[0] = memberVO;
		members[1] = memberVO2;
		members[2] = memberVO3;
		//출력도 메서드로 바로 호출하지 않고, 외부 클래스에서 객체로 만들어서 메서드를 호출
		MemberService memberService = new MemberService();
		//아래처럼 객체로 만들면, 호출(runtime)시 메모리에 로딩됨 -> 실행끝나면,반환
...
class MemberService {
	...
	public void printMember(MemberVO[] members) {
		// 멤버 서비스 클래스에서 퍼블릭 접근제어로 회원정보를 출력하는 메서드 생성
		// 향상된 for문을 사용
		for(MemberVO member:members) {
			System.out.println("회원의 이름은 "+member.getName()+" 회원의 나이는"+member.getAge()+" 폰번호는"+member.getPhoneNum());
		}
	}
	//회원을 출력하는 메서드를 생성
}