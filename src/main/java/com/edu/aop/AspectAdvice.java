...
public class AspectAdvice {
	...
	public Object sessionManager(ProceedingJoinPoint pjp) throws Throwable {
		...
		for(Object object:pjp.getArgs()) {
			if(object instanceof PageVO) { //AOP실행메서드 중 매개변수 PageVO pageVO 객체 판단
				//결과는 게시판, 멤버 서비스에 PageVO 사용하는 서비스에만 적용됨. 게시판생성관리에는 적용x
		...
		if(request != null) { // jsp에서 Get,Post 있을때
			...
			if(session.getAttribute("session_search_keyword") !=null) {
				...
				if(pageVO !=null) {//겟,셋 중에 Set할때 pageVO가 널이면 에러발생하기때문에 추가한 코드
			...
			if(session.getAttribute("session_board_type") !=null) {
				...
				if(pageVO !=null) { //set은 pageVO가 null이 아닐 경우만 실행되도록 구현
