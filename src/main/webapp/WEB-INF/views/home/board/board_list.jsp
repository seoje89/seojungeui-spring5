...
<div id="container">
	...
	<div class="bodytext_area box_inner">
		...
		<table class="bbsListTbl" summary="번호,제목,조회수,작성일 등을 제공하는 표">
			...
			<tbody>
				<c:forEach var="boardVO" items="${boardList}" varStatus="status">
					<tr>
						<td>
						<!-- 전체게시물-(현재페이지x1페이지당 보여줄 개수)+1페이지당 보여줄 개수-현재인덱스(위에서부터 0으로 시작) -->
						...
						<td><fmt:formatDate pattern="yyyy-MM-dd" value="${boardVO.reg_date}" /></td>
					</tr>
				</c:forEach>				
			</tbody>
		</table>
		...
		<!-- //페이징처리영역 -->
		<p class="btn_line">
		<!-- 등록 버튼은 로그인한 사용자만 보이도록 처리 -->
		<c:if test="${session_enabled}">
			<!-- 게시판이 공지사항일때는 관리자만 쓸수 있도록 처리, 공지사항 외에는 로그인한 사용자는 글쓰기 가능  -->
			<!-- 관리자일때, 일반사용자일때 1차조건으로 구분 / 공지사항이 아닐때 2차조건 -->
			<c:choose>
