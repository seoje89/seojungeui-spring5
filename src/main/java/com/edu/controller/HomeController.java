...
public class HomeController {
	...
	public String board_update(HttpServletRequest request, @RequestParam("file") MultipartFile[] files, BoardVO boardVO, RedirectAttributes rdat, PageVO pageVO) throws Exception {
		/* AOP로 구현
		...
		if(!boardVO.getWriter().equals(session.getAttribute("session_userid"))) {
			...
			rdat.addFlashAttribute("msgError","게시물은 본인만 수정이 가능합니다");
			//request객체에 이전페이지 URL("Referer")로 존재하고, 이 URL을 이전페이지 이동으로 사용
			return "redirect:"+request.getHeader("Referer"); //본인 ID의 글이 아닐때 뷰페이지로 이동
		}//else로 묶을 필요 없음. 위 return을 만나면 실행이 안되고 메서드가 종료되기 때문에
		*/
	...
	public String board_delete(PageVO pageVO, HttpServletRequest request, @RequestParam("bno") Integer bno, RedirectAttributes rdat) throws Exception {
		...
		if(!boardVO.getWriter().equals(session.getAttribute("session_userid"))) {
			...
			rdat.addFlashAttribute("msgError","게시물은 본인만 삭제 가능합니다");
			return "redirect:"+request.getHeader("Referer"); //본인 ID의 글이 아닐때 뷰페이지로 이동
		} //else로 묶을 필요 없음. return을 만나면 실행이 안되고 메서드가 종료되기떄문에
		*/
	...
	public String join_form() throws Exception {
		...
		//return "home/join"; // .jsp생략 , 타일즈 적용 전
		return "join.tiles"; // tiles폴더 안에 join.jsp 뷰 호출
	}
